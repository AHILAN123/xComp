    ...
    if 'model' in locals():
        ...
        if hasattr(m, 'forward') and callable(m.forward):
            ...
            for p in sig.parameters.values():
                if p.name not in ('self', 'cls'):
                    pytorch_args.append(p.name)
    with open("/home/tielixir/Coding/Projects/LLVMOptimizer/runs/run_affad946/pytorch_args.json", "w") as f:
...
try:
    ...
    sys.stderr = open("/home/tielixir/Coding/Projects/LLVMOptimizer/runs/run_affad946/trace.log", "w")
